...
// ✅ FIXED API 1: Get available staff & equipment for date/time
...
// ✅ FIXED API 2: Schedule operation with staff & equipment assignments
scheduleOperation: async (req, res) => {
    ...
    try {
        ...
        console.log('Received scheduled_start:', scheduled_start);
        ...
        // ✅ SIMPLE FIX: Direct datetime construction without complex parsing
        let sqlDateTime;
        
        // If scheduled_start already contains full datetime, use it directly
        ...
        // If we have separate date and time, combine them
            ...
            // Remove any timezone or milliseconds from scheduled_start if present
        ...
        if (scheduledDate < now) {
            return res.status(400).json({
                success: false,
                error: 'Cannot schedule operations in the past. Please select a future date and time.'
            });
        }
    ...
    // ✅ FIXED VERSION: Correct operation status calculation
getStaffDailySchedule: async (req, res) => {
    try {
        console.log('📅 Fetching staff daily schedule...');
        
        const staff_id = 2; // This is working for you
        ...
        // Get equipment details for each operation
        const assignmentsWithEquipment = await Promise.all(
            assignments.map(async (assignment) => {
                ...
                // Get team members for this operation
                ...
                return {
                    ...
                    team_count: teamMembers.length + 1 // +1 for current staff
                };
            })
        );
...
getWeeklyAssignmentCounts: async (req, res) => {
    try {
        ...
        // Get assignment counts with individual staff week operation count
        ...
        // Get total operations in the week
        ...
        // Calculate average operations per staff
